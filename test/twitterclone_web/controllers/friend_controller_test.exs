defmodule TwittercloneWeb.FriendControllerTest do
  use TwittercloneWeb.ConnCase

  alias Twitterclone.User

  @create_attrs %{"—no-schema": "some —no-schema"}
  @update_attrs %{"—no-schema": "some updated —no-schema"}
  @invalid_attrs %{"—no-schema": nil}

  def fixture(:friend) do
    {:ok, friend} = User.create_friend(@create_attrs)
    friend
  end
  ...
  describe "update friend" do
    ...
    test "redirects when data is valid", %{conn: conn, friend: friend} do
      ...
      assert html_response(conn, 200) =~ "some updated —no-schema"
    end
